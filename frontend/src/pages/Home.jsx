...
function Home() {
  ...
  const [campaignStats] = useState({ raised: 1840000, target: 2500000 });
  ...
  useEffect(() => {
    ...
    const fetchAppeal = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/urgent-appeal?raised=${campaignStats.raised}&target=${campaignStats.target}`);
        const data = await response.json();
  ...
  const currentDonation = recentDonations.length > 0 ? recentDonations[currentDonationIndex % recentDonations.length] : {
    name: "Aarav M.",
    amount: 1500,
    location: "Bengaluru",
    timeAgo: "4m ago"
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto px-4 py-6 pb-24 space-y-8 relative z-10">
        ...
        >
          <div className="text-center space-y-2">
            <h2 className="font-display font-extrabold text-2xl sm:text-3xl text-brand-navy tracking-tight">Comprehensive Elder Care Programs</h2>
            <p className="text-slate-500 text-sm">Specialized interventions addressing the spectrum of geriatric vulnerabilities.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div className="bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow group">
              ...
              <h3 className="font-bold text-slate-800 text-lg mb-2">Vision Restoration</h3>
              <p className="text-sm text-slate-600 leading-relaxed">Sponsoring cataract surgeries, intraocular lens implants, and prescription eyewear to prevent age-related blindness.</p>
            </div>
            <div className="bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow group">
              ...
              <h3 className="font-bold text-slate-800 text-lg mb-2">Dementia Care Unit</h3>
              <p className="text-sm text-slate-600 leading-relaxed">Memory clinics, cognitive behavioral therapy, and specialized caregivers for Alzheimer's and advanced dementia patients.</p>
            </div>
            <div className="bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow group">
              ...
              <h3 className="font-bold text-slate-800 text-lg mb-2">Elder Crisis Rescue</h3>
              <p className="text-sm text-slate-600 leading-relaxed">24/7 national emergency helpline, immediate legal aid, and secure shelter relocation for victims of elder abuse or sudden abandonment.</p>
            </div>
          </div>
        </motion.section>
        ...
        <motion.section 
          ...
          viewport={{ once: true, margin: "-50px" }} 
          transition={{ duration: 0.5, ease: "easeOut" }}
          className="bg-brand-navy rounded-3xl overflow-hidden shadow-lg text-white relative mt-8"
        >
          <div className="absolute inset-0 bg-brand-navy-dark opacity-50"></div>
          <div className="relative z-10 p-8 sm:p-12 text-center max-w-2xl mx-auto space-y-6">
            <div className="inline-flex items-center gap-2 text-xs font-bold text-brand-emerald bg-brand-emerald/10 px-4 py-2 rounded-xl border border-brand-emerald/30 uppercase tracking-widest">
              <span className="material-symbols-outlined text-[16px]">volunteer_activism</span>
              Companionship Initiative
            </div>
            <h2 className="text-3xl sm:text-4xl font-display font-extrabold tracking-tight">
              Eradicating the Epidemic of Elderly Loneliness
            </h2>
            <p className="text-slate-300 text-sm sm:text-base leading-relaxed">
              Beyond physical health, severe isolation accelerates cognitive decline. Our "Elder Companion" program matches vetted youth volunteers with homebound seniors for weekly visits, digital literacy training, and emotional support.
            </p>
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4">
              <button className="w-full sm:w-auto px-6 py-3 bg-white text-brand-navy font-bold rounded-xl hover:bg-slate-100 transition-colors shadow-lg flex justify-center items-center gap-2">
          ...
          <div className="relative z-10 space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-slate-100/80 pb-5">
              <div className="space-y-1.5">
                ...
                <h2 className="font-display font-extrabold text-xl sm:text-2xl text-brand-navy tracking-tight drop-shadow-sm">Elderly Winter Healthcare & Mobility Fund 2026</h2>
              </div>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-end">
                <div>
                  <span className="text-4xl sm:text-5xl font-display font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-brand-navy-dark to-brand-navy tracking-tight drop-shadow-sm">₹{campaignStats.raised.toLocaleString('en-IN')}</span>
                  <span className="text-sm text-slate-500 font-medium ml-3 block sm:inline mt-1 sm:mt-0">raised of <strong className="text-slate-700">₹{campaignStats.target.toLocaleString('en-IN')}</strong> target</span>
                </div>
                <span className="text-xs font-bold text-emerald-800 bg-gradient-to-r from-emerald-50 to-emerald-100/50 border border-emerald-200/50 px-3 py-1.5 rounded-lg shadow-sm">{(campaignStats.raised / campaignStats.target * 100).toFixed(1)}% Disbursed</span>
              </div>
              <div className="w-full bg-slate-100/80 h-4 rounded-full overflow-hidden shadow-inner border border-slate-200/50 p-0.5">
                <div className="bg-gradient-to-r from-brand-navy to-brand-emerald h-full rounded-full relative shadow-[0_0_10px_rgba(13,110,90,0.5)]" style={{ width: `${(campaignStats.raised / campaignStats.target * 100)}%` }}>
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <div className="text-[11px] text-slate-400/80 leading-relaxed space-y-4 text-center sm:text-left">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-white/5">
              <span>© 2026 SevaSparsh Foundation. All rights reserved.</span>
